...
[![Playwright Tests](https://github.com/Vranjan7077/playwright/actions/workflows/playwright.yml/badge.svg)](https://github.com/Vranjan7077/playwright/actions/workflows/playwright.yml)
...
[![Node](https://img.shields.io/badge/node-%3E%3D18.19-339933?logo=node.js&logoColor=white)](package.json)
...
```mermaid
flowchart LR
    A[Playwright Test Runner] --> B[fixtures/index.ts]
    B --> C[Page Object Model]
    C --> D([vranjan.dev — live site])
    B -.provides.-> E["formSubmissionGuard<br/>network interceptor"]
    B -.provides.-> F["makeAxeBuilder<br/>axe-core scans"]
    E -.protects.-> D
```
...
flowchart TD
    subgraph PR["Pull request"]
        direction LR
        L1[lint + typecheck] --> S1[smoke @chromium]
    end
    subgraph MAIN["Push to master · nightly 03:17 UTC · manual dispatch"]
        direction LR
        L2[lint + typecheck] --> R1[regression shard 1/4]
        L2 --> R2[regression shard 2/4]
        L2 --> R3[regression shard 3/4]
        L2 --> R4[regression shard 4/4]
        R1 & R2 & R3 & R4 --> M["merge blob reports<br/>into one HTML report"]
    end
```

- **Every PR**: lint + typecheck, then a fast `@smoke`-only run on chromium.
- **Push to `master`, nightly (03:17 UTC), and manual dispatch**: full
